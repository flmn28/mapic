...
RSpec.describe LikesController, type: :controller do
  ...
  describe "POST #like" do
    xit "create new like" do
      post :like, params: { location_id: @location.id }, session: valid_session
      expect(Like.all.count).to eq 1
      expect(Like.first.user_id).to eq 1
      expect(Like.first.location_id).to eq 1
    end

    xit "set correct location" do
      post :like, params: { location_id: @location.id }, session: valid_session
  ...
  describe "DELETE #unlike" do
    xit "destroy one like" do
      create :like, user_id: 1, location_id: 1
      delete :unlike, params: { location_id: @location.id }, session: valid_session
